...
use tuirealm::{
    props::{Color, Style, TextModifiers, TextSpan},
    tui::{style::ParseColorError, text::Span},
};
...
impl EntryTitle {
    pub fn for_entry(entry: &FeedEntry, theme_config: &ThemeConfig) -> Self {
        let text = format!(
            " {}{} ",
            if entry.starred { "🟊 " } else { "" },
            entry.title.clone()
        );
        let color = EntryTitle::resolve_color(entry, theme_config).unwrap_or_else(|err| {
    ...
    fn resolve_color(
        ...
        let color_str = match entry.status {
...
impl From<EntryTitle> for TextSpan {
    fn from(value: EntryTitle) -> Self {
        if value.read_status == ReadStatus::Unread {
            TextSpan::new(value.text.clone()).fg(value.color).bold()
        } else {
            TextSpan::new(value.text.clone()).fg(value.color).italic()
        }
    }
}
