use crate::{
    ...
    ui::{
        components::{
            error_message::ErrorMessage, feed_entry_list::FeedListViewType,
            keyboard_help::KeyboardHelp,
        },
...
impl Model {
    ...
    fn change_read_status(&mut self, entry_id: i32, new_status: ReadStatus) {
        ...
        tokio::spawn(async move {
            match miniflux_client
                .change_entry_read_status(entry_id, new_status)
                .await
            {
                Ok(_) => {}
    ...
    fn do_refresh(&mut self, view_type: FeedListViewType) {
        ...
        tokio::spawn(async move {
            ...
            let entries = match view_type {
                ...
                FeedListViewType::StarredEntries => {
                    miniflux_client.get_starred_entries(100, 0).await
                }
            };
            match entries {
                Ok(updated_entries) => {
                    let _ = messages_tx
                        .send(Message::FeedEntriesReceived(updated_entries))
                        .await;
                }
                Err(e) => Self::handle_error_message(e, messages_tx).await,
            }
        ...
        tokio::spawn(async move {
            match miniflux_client.fetch_original_content(entry_id).await {
                Ok(original_content_response) => {
                    let _ = messages_tx
                        .send(Message::OriginalEntryContentsReceived(
                            original_content_response,
                        ))
                        .await;
                }
                Err(e) => Self::handle_error_message(e, messages_tx).await,
    ...
    async fn handle_error_message(
        ...
        let _ = messages_tx
            .send(Message::RequestErrorEncountered(e.status(), e.to_string()))
            .await;
    }
}
...
impl Update<Message> for Model {
    fn update(&mut self, msg: Option<Message>) -> Option<Message> {
        if let Some(msg) = msg {
            ...
            match msg {
                Message::Batch(msgs) => {
                    let results: Vec<Option<Message>> = msgs
                        .iter()
                    ...
                    assert!(self
                        ...
                        .attr(
                            ...
                            tuirealm::AttrValue::String(format!(
                                "Error {status_code_str}: {err_string}"
                            ))
                        )
                        .is_ok());
